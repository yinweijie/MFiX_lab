...
if (mype == 0) open (unit=123,file="neighbors0.txt",action="write",status="replace")
if (mype == 1) open (unit=124,file="neighbors1.txt",action="write",status="replace")

DO LL = start_index, end_index
   CC_START = 1
   IF (LL.gt.1) CC_START = NEIGHBOR_INDEX(LL-1)
   CC_END   = NEIGHBOR_INDEX(LL)

   if (mype == 0) write (123,*) LL, IS_GHOST(LL), CC_START, CC_END-1
   if (mype == 1) write (124,*) LL, IS_GHOST(LL), CC_START, CC_END-1

   DO CC = CC_START, CC_END-1
      I  = NEIGHBORS(CC)
      if (mype == 0) write (123,*) "===================", I, IS_GHOST(I)
	  if (mype == 1) write (124,*) "===================", I, IS_GHOST(I)
   enddo
enddo

if (mype == 0) close (unit=123)
if (mype == 1) close (unit=124)
! stop

!-----------------------------------------------------------------------

if (mype == 0) open (unit=123,file="cord0.txt",action="write",status="replace")
if (mype == 1) open (unit=124,file="cord1.txt",action="write",status="replace")
...
DO LL = start_index, end_index
   ...
   if (mype == 0) then
		! IF(IS_GHOST(LL) .OR. IS_ENTERING_GHOST(LL) .OR. IS_EXITING_GHOST(LL)) then
		IF(.not. IS_normal(LL)) then 
			write (123,*) "is ghost: ", LL, DES_POS_NEW(LL,1), DES_POS_NEW(LL,2), &
				PMASS(LL), DES_RADIUS(LL), IS_GHOST(LL), IS_ENTERING_GHOST(LL), IS_EXITING_GHOST(LL)
		else
			write (123,*) "          ", LL, DES_POS_NEW(LL,1), DES_POS_NEW(LL,2), PMASS(LL), DES_RADIUS(LL)
		endif
   endif
   ...
   if (mype == 1) then
		! IF(IS_GHOST(LL) .OR. IS_ENTERING_GHOST(LL) .OR. IS_EXITING_GHOST(LL)) then
		IF(.not. IS_normal(LL)) then 
			write (124,*) "is ghost: ", LL, DES_POS_NEW(LL,1), DES_POS_NEW(LL,2), &
				PMASS(LL), DES_RADIUS(LL), IS_GHOST(LL), IS_ENTERING_GHOST(LL), IS_EXITING_GHOST(LL)
		else
			write (124,*) "          ", LL, DES_POS_NEW(LL,1), DES_POS_NEW(LL,2), PMASS(LL), DES_RADIUS(LL)
		endif
   endif
enddo
